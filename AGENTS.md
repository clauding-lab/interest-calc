...
12. **A successful `git push` is NOT a live deploy — verify the GitHub Pages build fired.** Pages legacy "deploy from branch" occasionally does NOT trigger on a push (silently — no error, no queued build). After any release push: `gh api repos/clauding-lab/interest-calc/pages/builds/latest --jq .commit` must equal `git rev-parse HEAD`; if not, force it with `gh api -X POST repos/clauding-lab/interest-calc/pages/builds`, then hash-check a changed asset live (`curl -s <url> | shasum -a 256` vs the local file). Never report a web change as live on the strength of the push alone. (See `AGENT_LEARNINGS.md` 2026-06-27.)
